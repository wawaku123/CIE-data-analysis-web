...
plt.rcParams["font.family"] = ["Microsoft YaHei", "SimHei", "WenQuanYi Micro Hei", "sans-serif"]
...
def calculate_original_color_center(df, bin_code_col='bin_code', ciex_col='ciex', ciey_col='ciey'):
    ...
    if bin_code_col not in df.columns or ciex_col not in df.columns or ciey_col not in df.columns:
        return None
    ...
    valid_mask = ~df[bin_code_col].isin(exclude_bins)

    # 排除ciex=0或ciey=0的点
    valid_mask &= (df[ciex_col] != 0) & (df[ciey_col] != 0)
    valid_mask &= (df[ciex_col] > 0.1) & (df[ciey_col] > 0.1)  # 同时排除异常小值
    ...
    vf1_excluded = len(df[df[bin_code_col] == 'VF1不良'])
    unlit_excluded = len(df[df[bin_code_col] == '未点亮'])
...
def main():
    ...
    if uploaded_file is not None:
            ...
            if original_center is not None:
                ...
                if len(valid_df) > 0:
                    ...
                    with col3:
                        ...
                        ax1.set_xlabel('ciex')
                        ax1.set_ylabel('ciey')
                        ax1.set_title(f'原始色点分布 (目标色区占比: {original_ratio:.2%})')
                        ...
                        legend1 = ax1.legend(
                            ...
                            framealpha=0.8,
                            borderpad=0.2,  # 减小内边距
                            handletextpad=0.1,  # 减小图例项与文字的间距
                            labelspacing=0.2,  # 减小标签间距
                            borderaxespad=0.3  # 减小图例边框与坐标轴的距离
                        )
                    ...
                    with col4:
                        ...
                        adjusted_points, actual_center_calc = adjust_color_points_with_center_shift(
                            points, original_center, target_center, concentration_gain, center_shift_gain
                        )
                        ...
                        ax2.set_xlabel('ciex')
                        ax2.set_ylabel('ciey')
                        ax2.set_title(f'调整后色点分布 (目标色区占比: {adjusted_ratio:.2%})')
                    ...
                    if st.button("确认应用色坐标调整", use_container_width=True):
                        ...
                        if details:
                            ...
                            st.write(f"- 目标色区占比提升: {original_ratio:.2%} → {adjusted_ratio:.2%}")
        ...
        # 4. 保存和下载
        st.subheader("4. 保存修改结果")

        if st.button("生成最终数据文件", use_container_width=True):
            # 获取修改后的数据
            df = st.session_state.current_df
            ...
            href = f'<a href="data:file/csv;base64,{b64}" download="{download_name}">📥 下载调整后的数据文件: {download_name}</a>'
