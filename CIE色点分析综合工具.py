...
def main():
    ...
    if 'uploaded_files' not in st.session_state:
        ...
        st.session_state.cell_size = 1.0  # 单元格大小
        ...
        if st.session_state.dataframes:
            ...
            with tab1:
                ...
                if all_ciex and all_ciey:
                    ...
                    # 创建自动范围选项
                    auto_range = st.checkbox("自动范围", value=True, on_change=update_chart, key="scatter_auto_range")

                    if not auto_range:
                        col1, col2 = st.columns(2)
                        with col1:
                            x_min_custom = st.number_input("X轴最小值", value=round(x_min, 3), step=0.001,
                                                           on_change=update_chart, key="scatter_x_min")
                            x_max_custom = st.number_input("X轴最大值", value=round(x_max, 3), step=0.001,
                                                           on_change=update_chart, key="scatter_x_max")
                        with col2:
                            y_min_custom = st.number_input("Y轴最小值", value=round(y_min, 3), step=0.001,
                                                           on_change=update_chart, key="scatter_y_min")
                            y_max_custom = st.number_input("Y轴最大值", value=round(y_max, 3), step=0.001,
                                                           on_change=update_chart, key="scatter_y_max")
                ...
                with st.spinner("正在生成图表..."):
                    ...
                    st.markdown(f"图表生成时间: {plot_time:.2f} 秒")
                    ...
                    with st.spinner(f"正在计算{'原始' if use_original_coords else '移动后'}数据的色区统计..."):
                        ...
                        for file_name, stats in zone_stats.items():
                            ...
                            fig = px.bar(
                                ...
                                hover_data=['点数', '占比(%)']  # 添加需要显示的悬停数据
                            )

                            # 自定义悬停模板，显示色区名称、数量和占比
                            fig.update_traces(
                    ...
                    with col1:
                        ...
                        if filter_outliers:
                            ciex_min = st.number_input("ciex 最小值", value=0.2, step=0.001, format="%.3f",
                                                       key="ciex_min")
                            ciex_max = st.number_input("ciex 最大值", value=0.8, step=0.001, format="%.3f",
                                                       key="ciex_max")
                            ciey_min = st.number_input("ciey 最小值", value=0.2, step=0.001, format="%.3f",
                                                       key="ciey_min")
                            ciey_max = st.number_input("ciey 最大值", value=0.8, step=0.001, format="%.3f",
                                                       key="ciey_max")
                        ...
                        if custom_color_range:
                            # 获取数据范围作为默认值
                            data_min = float(st.session_state.dataframes[material_file][value_column].min())
                            data_max = float(st.session_state.dataframes[material_file][value_column].max())
                            color_min = st.number_input("最小值", value=data_min, step=0.0001, format="%.4f",
                                                        key="color_min")
                            color_max = st.number_input("最大值", value=data_max, step=0.0001, format="%.4f",
                                                        key="color_max")
                        ...
                        cell_size = st.slider(
                            ...
                            min_value=0.5,
                            max_value=2.0,
                            value=st.session_state.get("cell_size", 1.0),
                            step=0.1,
                    ...
                    with col2:
                        ...
                        if st.button("生成交互Mapping图", key="generate_map_button"):
                            with st.spinner("正在生成Mapping图..."):
                                title = f'{material_file} - {value_column} Mapping图 ({product_type_mapping})'
                                if st.session_state.ncsp_region == 1:
                                    title += " - 区域1"
                                elif st.session_state.ncsp_region == 2:
                                    title += " - 区域2"

                                fig = generate_interactive_mapping_plot(
